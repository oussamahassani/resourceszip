...
const AllMigrations = () => {
  ...
  const load = (page = 1) => {
    setLoading(true)
    axiosInstance.get('/api/operationAbonnement/alldemandesmigration', { params: { page: page - 1, size: 20 } })
      .then(res => {
        const d = res.data
        setData(d?.content || d || [])
  ...
  const columns = [
    ...
    { header: 'Ancienne offre', render: (r) => r.ancienneOffre?.offreName || '-' },
    { header: 'Nouvelle offre', render: (r) => r.nouvelleOffre?.offreName || '-' },
  ...
  return (
    <section className="content">
      <PageHeader title="Demandes de migration" breadcrumb={[{ label: 'Opérations' }, { label: 'Migration' }]} />
      <div className="container-fluid">
        <div className="card shadow mb-4">
          <div className="card-header">
            <h6 className="m-0 font-weight-bold" style={{ color: '#590E9C' }}>Demandes de migration</h6>
          </div>
          <div className="card-body">
            <DataTable columns={columns} data={data} loading={loading}
              pagination={pagination} onPageChange={load} emptyMessage="Aucune demande de migration" />
          </div>
        </div>
      </div>
    </section>
