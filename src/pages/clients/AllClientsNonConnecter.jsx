...
const AllClientsNonConnecter = () => {
  ...
  const load = (page = 1) => {
    setLoading(true)
    clientsService.getNonConnecter({ page: page - 1, size: 20 })
      .then(data => {
  ...
  const columns = [
    ...
    { header: 'Date abonnement', render: (r) => formatDate(r.dateAbonnement) },
    {
      header: 'Actions',
      render: (r) => (
  ...
  return (
    <section className="content">
      <PageHeader title="Clients non connectés" breadcrumb={[{ label: 'Clients', path: '/client/allclients' }, { label: 'Non connectés' }]} />
      <div className="container-fluid">
        <div className="card shadow mb-4">
          <div className="card-header">
            <h6 className="m-0 font-weight-bold" style={{ color: '#590E9C' }}>Clients non connectés</h6>
          </div>
          <div className="card-body">
            <DataTable columns={columns} data={clients} loading={loading}
              pagination={pagination} onPageChange={load} emptyMessage="Aucun client non connecté" />
          </div>
        </div>
      </div>
    </section>
...
export default AllClientsNonConnecter
