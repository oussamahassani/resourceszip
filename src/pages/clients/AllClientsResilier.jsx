...
const AllClientsResilier = () => {
  ...
  const load = (page = 1) => {
    setLoading(true)
    clientsService.getResilie({ page: page - 1, size: 20 })
      .then(data => {
  ...
  const columns = [
    ...
    { header: 'Date résiliation', render: (r) => formatDate(r.dateResiliation) },
    {
      header: 'Actions',
      render: (r) => (
  ...
  return (
    <section className="content">
      <PageHeader title="Clients résiliés" breadcrumb={[{ label: 'Clients', path: '/client/allclients' }, { label: 'Résiliés' }]} />
      <div className="container-fluid">
        <div className="card shadow mb-4">
          <div className="card-header">
            <h6 className="m-0 font-weight-bold text-danger">Clients résiliés</h6>
          </div>
          <div className="card-body">
            <DataTable columns={columns} data={clients} loading={loading}
              pagination={pagination} onPageChange={load} emptyMessage="Aucun client résilié" />
          </div>
        </div>
      </div>
    </section>
...
export default AllClientsResilier
