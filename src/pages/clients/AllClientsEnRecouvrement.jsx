...
const AllClientsEnRecouvrement = () => {
  ...
  const load = (page = 1) => {
    setLoading(true)
    clientsService.getEnRecouvrement({ page: page - 1, size: 20 })
      .then(data => {
  ...
  const columns = [
    ...
    { header: 'Montant dû', render: (r) => r.montantDu ? `${r.montantDu} DT` : '-' },
    {
      header: 'Actions',
      render: (r) => (
  ...
  return (
    <section className="content">
      <PageHeader title="Clients en recouvrement" breadcrumb={[{ label: 'Clients', path: '/client/allclients' }, { label: 'En recouvrement' }]} />
      <div className="container-fluid">
        <div className="card shadow mb-4">
          <div className="card-header">
            <h6 className="m-0 font-weight-bold text-warning">Clients en recouvrement</h6>
          </div>
          <div className="card-body">
            <DataTable columns={columns} data={clients} loading={loading}
              pagination={pagination} onPageChange={load} emptyMessage="Aucun client en recouvrement" />
          </div>
        </div>
      </div>
    </section>
...
export default AllClientsEnRecouvrement
